...
- **Cross-reference superpowers files with embedded markdown links** wherever a curious reader would benefit from the source: `See the [original spec](docs/superpowers/specs/2026-04-02-foo-design.md) for the rejected alternatives.` Use **relative paths** (relative to the project root, not to `docs/manual/`); the HTML viewer resolves them via the project-root directory handle. Use the artifact's title (or a meaningful phrase) as link text — never raw paths.
...
- **New** → add a row. `First cited` and `Last seen` both = `python3 manual_helper.py now-et`. Hash = the value from `diff-artifacts`. Title = the artifact's H1. Path is wrapped as a markdown link: `[2026-04-02-foo-design.md](docs/superpowers/specs/2026-04-02-foo-design.md)` so the table is clickable in any markdown renderer.
